...
final class PasscodeFlowTests: XCTestCase {
    ...
    func testAppPasscodeSetupView_confirmStep_showsSecondHeadline() throws {
        ...
        XCTAssertNoThrow(try view.inspect().find(text: L("비밀번호 다시 입력")))
        XCTAssertNoThrow(try view.inspect().find(text: L("확인을 위해 한 번 더 입력해주세요.")))
    }
    ...
    func testAppPasscodeSetupView_withErrorMessage_showsErrorText() throws {
        ...
        XCTAssertNoThrow(try view.inspect().find(text: L("비밀번호가 달라요. 처음부터 다시 입력해주세요.")))
    }
    ...
    func testAppPasscodeEntryView_withError_showsErrorText() throws {
        ...
        XCTAssertNoThrow(try view.inspect().find(text: L("비밀번호가 달라요. 다시 입력해주세요.")))
    }
